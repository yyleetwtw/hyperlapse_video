...
function [H, inliers,state] = EstimateHomographyByRANSAC(x1, x2, t)
    state = 0; % 0:no problem, 1:not enough inliner
    
    % x1,x2 be row vector with dim 2 or 3 (column # = inlier #)
    ...
    if npts < 4
        fprintf('Must have at least 4 points to fit homography \n');
        state = 1;
        H = [];
        inliers = [];
        return;
    ...
    if rows == 2    % pad 3rd for homography
        x1 = [x1; ones(1,npts)];
        x2 = [x2; ones(1,npts)];        
    end
    ...
    [H, inliers,ransac_state] = RANSAC([x1; x2], DLTfn, errorfn, degenfn, s, t);
    if ransac_state ~= 0 
        state = 1;
        return
    end
    % this function is used to fit homography given inliers
