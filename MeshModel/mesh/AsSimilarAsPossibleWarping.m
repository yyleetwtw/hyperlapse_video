classdef AsSimilarAsPossibleWarping < handle
    ...
    properties
        %smooth constraints
        SmoothConstraints;
        SCc; % Smooth Constraint count, used as index to set SmoothConstrains
        num_smooth_cons;
    ...
    methods (Access = public)
        function obj = AsSimilarAsPossibleWarping(height,width,quadWidth,quadHeight,weight)
            ...
            % indexing grid points in linear way
            obj.x_index = 0:obj.height*obj.width-1;
            obj.y_index = obj.height*obj.width:2*obj.height*obj.width-1;
            %   internal grid vertices , boundary , corner, something wrong
            %   here !? SCc inconsistent with num_smooth_cons
        ...
        function SetControlPts(obj,inputsPts,outputsPts)
            ...
            for i=1:len
               ...
               obj.dataterm_element_i(i) = floor(pt.y/obj.quadHeight)+1; % find feature in which grid index
               ...
               coefficients = qd.getBilinearCoordinates(pt); % determine how to interpolate pt with grid vertices
        ...
        function Solve(obj)
             ...
             % b =series of 0 of smooth constraint, following by the feature points coordinate in destination
             % put in the way [f1_x f1_y f2_x f2_y ...]'
             ...
             for i=1:length(obj.SmoothConstraints)
                ...
                AVals(cc) = obj.SmoothConstraints(i,3); % mul each term with alpha here ??. this case corresponding to alpha=1 ??
                cc = cc + 1;
             end
             ...
             x = A\b; % x is the position of transformed grid vertices
             % A is the mat that interpolate new feature coordinate using x
             % data term corresponding to ||Ax - orig_feature_in_dest||
        ...
        function warp = Warp(obj,Img,gap) % Img = prev(src)
        ...
        function homos = CalcHomos(obj)
            homos = zeros(obj.height,obj.width,3,3); % each cell have a homography
            
            for i=1:obj.height-1
               for j=1:obj.width-1
                   q1 = obj.source.getQuad(i,j); % cell corner vertex
    ...
    methods (Access = private) %functions of solving as similar as possible warping
        ...
        function CreateSmoothCons(obj,weight) % weight = lambda
           obj.rowCount = 0;
           % 4 corners set grid weight
           i=0;j=0;
        ...
        function b = CreateDataCons(obj)
           ...
           for k=1:len
               ...
               v00 = obj.dataterm_element_V00(k); % these are bilinear weighting corresponding its grid vertex
...
%             rowCount = rowCount + 1;
    ...
    methods (Access = private)%functions of warping
        function quadWarp(obj,im,q1,q2)
             ...
             source = zeros(4,2);
             target = zeros(4,2);
             % DONT KNOW WHY src, target inverse here (out caller
             % src=q1,dst=q2)
             % ANS: transform strangely shape back to 1:fwidth,1:fheight
             % some pt can't be interpolated using warped img, will be
             % black
