    ...
    methods 
        function obj = Mesh(rows,cols,quadWidth,quadHeight)
           ...
           ySet = 0;
            ...
            x = 1;i = 1;
            while obj.imgCols - x > 0.5*quadWidth % add mesh as long as the remaining col# > 0.5*meshWidtg
                xSet(i) = x;
                i = i+1;
                x = x + obj.quadWidth;
            end
            xSet(i) = obj.imgCols; % xSet saves the starting col pos of every mesh
            ...
            obj.meshWidth = length(xSet); % actual contained # of mesh in row & col
            obj.meshHeight =length(ySet); % in addition with end point (num mesh points)
        ...
        function meshImg = drawMesh(obj,Img,gap)
          ...
          meshImg = step(shapeLine,meshImg,uint16(Lines)); % this draws line on img
          meshImg = step(shapePoints,meshImg,uint16(Points)); % this draws pt on img
